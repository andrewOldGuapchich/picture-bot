...
class SimpleKotlinBot : TelegramLongPollingBot() {
    ...
    init {
        ...
        subscriberService.allUser().forEach { user ->
            sendMessage(
                user, "Бот был перезапущен по техническим причинам.\nПриносим свои извинения.\n\nВы получите следующее фото через 15 минут."
            )
    ...
    private fun sendWelcomeMessage(chatId: String) {
        val welcomeText = "🌟 Добро пожаловать! 🌟\n" +
            "Я бот, который будет радовать вас красивыми фотографиями!\n\n" +
            "📸 Начиная с этого момента, я буду присылать вам случайные фото каждые 15 минут.\n\n" +
    ...
    private fun sendStatus(chatId: String) {
        ...
        val statusText = "📊 Статус подписки\n\n" +
                "$status\n"
            "Всего подписчиков: $totalSubscribers\n\n" +
                    if (isSubscribed) "Следующее фото будет через 15 минут"
                    else "Отправьте /start для подписки"
    ...
    private fun sendHelp(chatId: String) {
        val helpText = "🤖 PictureBot - Помощь\n\n" +
